...
/*   Updated: 2025/03/28 14:00:31 by aadyan           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static int	mandelbrot(t_complex_numbers *c_nums)
{
	double	z_x;
	double	z_y;
	double	tmp;
...
void	draw_mandelbrot(t_data *img, t_properties props)
{
	...
	while (y < WIN_SIZE - 1)
	{
		...
		while (x < WIN_SIZE - 1)
		{
			...
			iter = mandelbrot(c_nums);
			free(c_nums);
			c_nums = get_complex_nums(x + 1, y, props);
			iter_next = mandelbrot(c_nums);
			free(c_nums);
			if (abs(iter - iter_next) > 1)
				draw_pixel(x, y, img, 0xFFFFFF);
			++x;
		}
		++y;
	}
